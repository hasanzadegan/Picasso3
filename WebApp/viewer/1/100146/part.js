...
app = angular.module("APP").controller("CtrlPart", ['$rootScope', '$scope','$pages','$contents',
    function ($rootScope, $scope,$pages,$contents) {
		...
		$rootScope.navigateURL = function(partIndex,pageId,pageContetId){
            var data = {};
            data.partIndex = partIndex;
            data.pageId = pageId;
            data.pageContetId = pageContetId;
            $rootScope.$broadcast("outsideChangePath",data);
        }

        $scope.$on("outsideChangePath",function (evt,data) {
            if($scope.partIndex==data.partIndex)
                $scope.navigateULR(data.pageId,data.pageContetId);
        });
