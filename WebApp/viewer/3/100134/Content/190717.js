app = angular.module("APP").controller("Ctrl190717", ['$rootScope','$scope', '$http','$q','$filter','$translate','$mdToast','$http','NgTableParams', function ($rootScope,$scope, $http,$q, $filter,$translate,$mdToast,$http,NgTableParams){
	...
	//End For Dialog
 	...
 	$rootScope.tables ={};
	$rootScope.setTableData = function(rows,cols,tableName)
	{
		$rootScope.tables[tableName] = new NgTableParams({}, { counts: [],dataset: rows});
		$rootScope.columns = cols;
	}
 
}]);
app.requires.push('ngMaterial','ngStorage','oc.lazyLoad','pascalprecht.translate','ngMessages','ngMask','ngTable');
